      ...
      distribution.groupB.status.get("nid", (e, v) => {
        try {
          expect(Object.values(v)).toEqual(
            expect.arrayContaining(allNodes.map((node) => id.getNID(node)))
          );
          done();
        } catch (error) {
          done(error);
    ...
    distribution.groupC.groups.put(config, groupC, (e, v) => {
      ...
      distribution.local.comm.send(["groupC", n1], remote, (e, v) => {
        distribution.groupC.groups.get("groupC", (e, v) => {
          ...
          try {
            expect(Object.keys(n2View)).toEqual(
              expect.arrayContaining([id.getSID(n1), id.getSID(n2)])
            );
            expect(Object.keys(n1View)).toEqual(
              expect.arrayContaining([id.getSID(n2)])
            );
            done();
          } catch (error) {
            done(error);
